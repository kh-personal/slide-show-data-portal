...
The system SHALL render a shared header row for units 01 through 08 and 8 unit squares per floor row. Each populated unit square SHALL display entry time, exit time, pax count, and CAS staff count without repeating the unit label inside the cell.

#### Scenario: Unit has movement data

- **WHEN** movement data exists for a floor and unit in the selected house
- **THEN** the corresponding unit square displays the entry time, exit time, pax count, and CAS staff count
...
- **THEN** the grid displays one header row labeled `Unit 01` through `Unit 08`
